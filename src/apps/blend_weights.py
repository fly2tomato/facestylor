...
def swap_weights(base_ckpt, swap_ckpt, start_index, end_index=8):
    for i in range(start_index, end_index):
        for layer in base_ckpt.keys():
            if f'convs.{2*i}' in layer or f'convs.{2*i + 1}' in layer or f'to_rgbs.{i}' in layer:
...
def main():
    ...
    modelA.decoder.load_state_dict(swap_weights(base_ckpt, swap_ckpt, start_index, end_index))
